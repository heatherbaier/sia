...
if __name__ == "__main__":
    ...
    if args.nodups:
        labels_path = f"/sciclone/geograd/heather_data/ti/data_new/{args.region}_ys_nodups.json"
    else:
...
#     Load transformation stats
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(target_dataset)):
        ...
        train_loader = DataLoader(target_dataset, batch_size = args.bs, sampler = train_sampler, num_workers = 0)
        val_loader = DataLoader(target_dataset, batch_size = args.bs, sampler = val_sampler, num_workers = 0)
