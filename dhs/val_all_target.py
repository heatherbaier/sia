...
#     parser.add_argument('--labels_path', required = False, default = "/sciclone/geograd/Heather/c1/data/clean/c2_ys.json")
    ...
    labels_path = f"/sciclone/geograd/heather_data/ti/data_new/{args.region}_ys{nodups}.json"
    coords_path = f"/sciclone/geograd/heather_data/ti/data_new/{args.region}_coords.json"
    lc_path = f"/sciclone/geograd/heather_data/ti/lc_jsons/{args.region}_lcs.json"
    ...
    # Hook to capture the output of the desired layer
    conv1_fc1_outputs = []
    
    def hook_fn(module, input, output):
        s = list(output.cpu().detach().numpy())
        s = [float(x) for x in s]
        conv1_fc1_outputs.append(s)  # Capture output and move to CPU
    
    # Register the hook
    model.conv1.hypernet.fc1.register_forward_hook(hook_fn)    
    ...
    for count, (inputs, targets, coords, imname) in enumerate(target_dataset):
        ...
        if count % 2500 == 0:
            ...
            to_save = dict(zip(ns, conv1_fc1_outputs))            
            with open(f"{folder_name}/kfold{args.kfold}/{args.target_name}_results/epoch{epoch}_hypernet{nodups}_fcs.json", "w") as f:
                json.dump(to_save, f)
